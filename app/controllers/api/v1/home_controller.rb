module Api
  module V1
    class HomeController < ApplicationController
      def index
        # Debug logging
        Rails.logger.info "🔍 HOME API DEBUG - Starting home data fetch"
        ...
        # Log counts
        Rails.logger.info "📊 HOME API DATA COUNTS:"
        Rails.logger.info "  - Categories: #{categories.count}"
        Rails.logger.info "  - Best Sellers: #{best_sellers.count}"
        Rails.logger.info "  - Hot Special Offer: #{hot_special_offer.count}"
        Rails.logger.info "  - Essential Accessories: #{essential_accessories.count}"
        Rails.logger.info "  - Early Order - Trending: #{early_order_products[:trending_products].count}"
        Rails.logger.info "  - Early Order - New: #{early_order_products[:new_products].count}"
        Rails.logger.info "  - Early Order - Ending Soon: #{early_order_products[:ending_soon_products].count}"
        Rails.logger.info "  - Early Order - Arriving Soon: #{early_order_products[:arriving_soon_products].count}"
